 ...
 *
 * The runtime serializes the similarity score as `_score`, and omits `matches`,
 * `data`, `primary_key` and `metadata` entirely when they are empty. Consumers of
 * `SearchMatch` expect `score` and expect the four objects to always be present,
 * so both are reconciled here.
 */
export function normalizeSearchMatch(match: WireSearchMatch): SearchMatch {
  return {
