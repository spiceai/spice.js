...
exports.GrpcFlightClient = void 0;
exports.splitArrowIpcStream = splitArrowIpcStream;
...
const client_1 = require("../adbc/client");
...
function arrowMessageBodyLength(header) {
    return apache_arrow_1.Message.decode(header).bodyLength;
}
...
const PACKAGE_PATH = __dirname.includes('.next')
    ? path_1.default.join(__dirname.substring(0, __dirname.indexOf('.next')), './node_modules/@spiceai/spice/dist/node/')
...
async function downloadProtoFile(logger) {
    try {
        ...
        const response = await node_1.platform.fetch(PROTO_DOWNLOAD_URL, {
            method: 'GET',
            headers: {},
        });
        if (!response.ok) {
...
class GrpcFlightClient {
    ...
    constructor(apiKey, flightUrl, userAgent, flightTlsEnabled, logger, tlsClientCertFile, tlsClientKeyFile, tlsRootCertFile) {
        ...
        this.logger = logger || new logger_1.Logger(true);
    ...
    async executePreparedStatement(client, queryText, parameters) {
        const created = await this.doAction(client, client_1.FlightSqlActions.CreatePreparedStatement, (0, client_1.encodeCreatePreparedStatementRequest)(queryText));
        if (created.length === 0) {
            throw new Error('Failed to prepare the query: the runtime returned no prepared statement');
        }
        const { preparedStatementHandle } = (0, client_1.decodeCreatePreparedStatementResult)(Buffer.from(created[0].body));
        ...
        try {
            const ipc = Array.isArray(parameters)
                ? (0, client_1.serializeParametersToIPC)(parameters)
                : (0, client_1.serializeNamedParametersToIPC)(parameters);
            handle = await this.bindParameters(client, handle, ipc);
            const ticket = await new Promise((resolve, reject) => {
                client.GetFlightInfo({
                    type: flight_1.DescriptorType.CMD,
                    cmd: (0, client_1.encodeCommandPreparedStatementQuery)(handle),
                }, (err, result) => {
                    if (err) {
                        reject(err);
                        return;
            ...
            const close = () => {
                this.doAction(client, client_1.FlightSqlActions.ClosePreparedStatement, (0, client_1.encodeClosePreparedStatementRequest)(handle)).catch(() => {
            ...
            await this.doAction(client, client_1.FlightSqlActions.ClosePreparedStatement, (0, client_1.encodeClosePreparedStatementRequest)(handle)).catch(() => {
        ...
        return new Promise((resolve, reject) => {
            ...
            call.on('end', () => {
                ...
                if (metadata && metadata.length > 0) {
                    try {
                        const updated = (0, client_1.decodeCreatePreparedStatementResult)(Buffer.from(metadata));
            ...
            const cmd = (0, client_1.encodeCommandPreparedStatementQuery)(handle);
            ...
            messages.forEach((message, index) => {
                call.write({
                    ...
                    ...(index === 0
                        ? { flightDescriptor: { type: flight_1.DescriptorType.CMD, cmd } }
                        : {}),
    ...
    async executeQuery(queryText, parameters, headers) {
        ...
        const flightTicket = await new Promise((resolve, reject) => {
            client.GetFlightInfo({ type: flight_1.DescriptorType.CMD, cmd: commandBuff }, (err, result) => {
                if (err) {
                    reject(err);
                    return;
                }
...
exports.GrpcFlightClient = GrpcFlightClient;
