...
import { getIpcMessage } from './flight';
import { AsyncQuery, } from './async-query';
import { jsonToArrowTable, convertToSqlV1Format, normalizeSchema, serializeArrowField, } from './arrow-utils';
        ...
        try {
            ...
            resultStream.on('data', (response) => {
                ...
                if (!schema) {
                    ...
                    const chunkTable = wrapTableForDecimalConversion(tableFromIPC([schema, ipcMessage]));
                    onData(chunkTable);
                }
            });
        ...
        for (const line of lines) {
            try {
                ...
                if (sqlV1.data.length > 0) {
                    ...
                    if (onData) {
                        const partialTable = wrapTableForDecimalConversion(jsonToArrowTable(schema, sqlV1.data));
                        onData(partialTable);
                    }
                }
            }
